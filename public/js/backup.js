...
async function verificarPermissaoBackup() {
  try {
    ...
    if (!res.ok) {
      // 401 = token inválido → redireciona login
      if (res.status === 401) { window.location.href = "index.html"; return; }
      // qualquer outro erro de servidor → mostra conteúdo mesmo assim (master pode estar sem acesso salvo)
      console.warn("meu-acesso retornou", res.status, "— mostrando conteúdo mesmo assim");
      return;
    }
    const data = await res.json();
    if (data.acesso === false) mostrarSemPermissao();
  } catch (e) {
    // Erro de rede — não esconde o conteúdo, apenas loga
    console.warn("Não foi possível verificar permissão de backup:", e.message);
  }
}
...
async function carregarClientes() {
  try {
    ...
    console.log(`[backup] Clientes carregados: ${clientesCache.length} total, ${clientesCache.filter(c=>c.backupHabilitado).length} habilitados`);
  } catch (e) {
...
async function carregarBackups() {
  ...
  try {
    ...
    // Filtra por habilitados apenas se houver clienteId específico no filtro
    // Sem filtro de cliente → mostra todos os registros existentes
    if (filtroClienteId) {
      backupsCache = todos.filter(b => String(b.clienteId?._id || b.clienteId || "") === filtroClienteId);
    } else {
      const habIds = new Set(clientesCache.filter(c => c.backupHabilitado).map(c => String(c._id)));
      backupsCache = habIds.size === 0 ? todos : todos.filter(b => habIds.has(String(b.clienteId?._id || b.clienteId || "")));
    }
