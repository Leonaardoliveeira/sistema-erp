...
function verificarLogin() {
    ...
    if (menuBackup) {
        // 1. Esconder totalmente por padrão usando display
        menuBackup.style.display = "none";

        if (usuario.perfil === "master") {
            menuBackup.style.display = "";
        } else {
            // 2. Chamar a rota correta do servidor
            fetch("/api/backup-permissao", {
                ...
                .then(d => {
                    if (d.visualizar) {
                        menuBackup.style.display = ""; // Mostra se tiver permissão
                    } else {
                        // Se estiver na página de backup sem permissão, expulsa
